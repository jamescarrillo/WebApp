 ...
 * @author alabajos
 */
@WebServlet(name = "ViaticosPrint", urlPatterns = {"/ViaticosPrint"})
public class ViaticosPrint extends HttpServlet {
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            switch (formato) {
                case "pdf":
                    byte[] bs = JasperExportManager.exportReportToPdf(crearReporte(anho,mes));
                    ...
                    break;
                case "word":
                    JasperPrint print1 = crearReporte(anho,mes);
                    ...
                    exportador1.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica1 + "/viatico.doc");
                    exportador1.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador1.exportReport();
                    FileInputStream entrada1 = new FileInputStream(rutafisica1 + "/viatico.doc");
                    byte[] lectura1 = new byte[entrada1.available()];
                    entrada1.read(lectura1);
                    response.setContentType("application/word");
                    response.setHeader("Content-Disposition", "attachment; filename=viatico.doc");
                ...
                case "excel":
                    JasperPrint print2 = crearReporte(anho,mes);
                    ...
                    exportador.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica + "/viatico.xls");
                    ...
                    FileInputStream entrada = new FileInputStream(rutafisica + "/viatico.xls");
                    byte[] lectura = new byte[entrada.available()];
                    entrada.read(lectura);
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename = viatico.xls");
                    ...
                    break;
                case "odt":
                    JasperPrint print3 = crearReporte(anho,mes);
                    ...
                    exportador2.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica2 + "/viatico.odt");
                    exportador2.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador2.exportReport();
                    FileInputStream entrada2 = new FileInputStream(rutafisica2 + "/viatico.odt");
                    byte[] lectura2 = new byte[entrada2.available()];
                    entrada2.read(lectura2);
                    response.setContentType("application/odt");
                    response.setHeader("Content-Disposition", "attachment; filename=viatico.odt");
            ...
            Logger.getLogger(ViaticosPrint.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public JasperPrint crearReporte(String anho, String mes) throws JRException, SQLException, ServletException, ClassNotFoundException, ParserConfigurationException, SAXException, IOException {
        ServletContext srvcon = getServletContext();
        String rutafisica = srvcon.getRealPath("//reportes//");

        JasperReport masterReport = (JasperReport) JRLoader.loadObject(rutafisica + "/viatico.jasper");
