 ...
 * @author alabajos
 */
@WebServlet(name = "ResolucionPrint", urlPatterns = {"/ResolucionPrint"})
public class ResolucionPrint extends HttpServlet {
        ...
        try {
            ...
            switch (formato) {
                ...
                case "word":
                    ...
                    exportador1.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica1 + "/resolucion.doc");
                    exportador1.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador1.exportReport();
                    FileInputStream entrada1 = new FileInputStream(rutafisica1 + "/resolucion.doc");
                    byte[] lectura1 = new byte[entrada1.available()];
                    entrada1.read(lectura1);
                    response.setContentType("application/word");
                    response.setHeader("Content-Disposition", "attachment; filename=resolucion.doc");
                ...
                case "excel":
                    ...
                    exportador.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica + "/resolucion.xls");
                    ...
                    FileInputStream entrada = new FileInputStream(rutafisica + "/resolucion.xls");
                    byte[] lectura = new byte[entrada.available()];
                    entrada.read(lectura);
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename = resolucion.xls");
                ...
                case "odt":
                    ...
                    exportador2.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica2 + "/resolucion.odt");
                    exportador2.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador2.exportReport();
                    FileInputStream entrada2 = new FileInputStream(rutafisica2 + "/resolucion.odt");
                    byte[] lectura2 = new byte[entrada2.available()];
                    entrada2.read(lectura2);
                    response.setContentType("application/odt");
                    response.setHeader("Content-Disposition", "attachment; filename=resolucion.odt");
            ...
            Logger.getLogger(ResolucionPrint.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public JasperPrint crearReporte(String anho) throws JRException, SQLException, ServletException, ClassNotFoundException, ParserConfigurationException, SAXException, IOException {
       ServletContext srvcon = getServletContext();
        ...
        JasperReport masterReport = (JasperReport) JRLoader.loadObject(rutafisica + "/resolucion.jasper");
