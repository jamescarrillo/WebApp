 ...
 * @author alabajos
 */
@WebServlet(name = "OrdenServicioPrint", urlPatterns = {"/OrdenServicioPrint"})
public class OrdenServicioPrint extends HttpServlet {
    public Integer mes;
        ...
        try {
            ...
            switch (formato) {
                ...
                case "word":
                    ...
                    exportador1.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica1 + "/servicio.doc");
                    exportador1.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador1.exportReport();
                    FileInputStream entrada1 = new FileInputStream(rutafisica1 + "/servicio.doc");
                    byte[] lectura1 = new byte[entrada1.available()];
                    entrada1.read(lectura1);
                    response.setContentType("application/word");
                    response.setHeader("Content-Disposition", "attachment; filename=servicio.doc");
                ...
                case "excel":
                    ...
                    exportador.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica + "/servicio.xls");
                    ...
                    FileInputStream entrada = new FileInputStream(rutafisica + "/servicio.xls");
                    byte[] lectura = new byte[entrada.available()];
                    entrada.read(lectura);
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename = servicio.xls");
                ...
                case "odt":
                    ...
                    exportador2.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica2 + "/servicio.odt");
                    exportador2.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador2.exportReport();
                    FileInputStream entrada2 = new FileInputStream(rutafisica2 + "/servicio.odt");
                    byte[] lectura2 = new byte[entrada2.available()];
                    entrada2.read(lectura2);
                    response.setContentType("application/odt");
                    response.setHeader("Content-Disposition", "attachment; filename=servicio.odt");
            ...
            Logger.getLogger(OrdenServicioPrint.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
    ...
    public JasperPrint crearReporte(String anho) throws JRException, SQLException, ServletException, ClassNotFoundException, ParserConfigurationException, SAXException, IOException {
        ...
        JasperReport masterReport = (JasperReport) JRLoader.loadObject(rutafisica + "/servicio.jasper");
