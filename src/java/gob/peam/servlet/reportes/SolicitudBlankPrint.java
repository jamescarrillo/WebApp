 ...
 * @author alabajos
 */
@WebServlet(name = "SolicitudBlankPrint", urlPatterns = {"/SolicitudBlankPrint"})
public class SolicitudBlankPrint extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            switch (formato) {
                case "pdf":
                    byte[] bs = JasperExportManager.exportReportToPdf(crearReporte(telefono, nombre, correo, dependencia, solicitud, dni));
                    ...
                    break;
                case "word":
                    JasperPrint print1 = crearReporte(telefono, nombre, correo, dependencia, solicitud, dni);
                    ...
                    exportador1.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica1 + "/solicitud.doc");
                    exportador1.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador1.exportReport();
                    FileInputStream entrada1 = new FileInputStream(rutafisica1 + "/solicitud.doc");
                    byte[] lectura1 = new byte[entrada1.available()];
                    entrada1.read(lectura1);
                    response.setContentType("application/word");
                    response.setHeader("Content-Disposition", "attachment; filename=solicitud.doc");
                ...
                case "excel":
                    JasperPrint print2 = crearReporte(telefono, nombre, correo, dependencia, solicitud, dni);
                    ...
                    exportador.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica + "/solicitud.xls");
                    ...
                    FileInputStream entrada = new FileInputStream(rutafisica + "/solicitud.xls");
                    byte[] lectura = new byte[entrada.available()];
                    entrada.read(lectura);
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename = solicitud.xls");
                ...
                case "odt":
                    JasperPrint print3 = crearReporte(telefono, nombre, correo, dependencia, solicitud, dni);
                    ...
                    exportador2.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica2 + "/solicitud.odt");
                    exportador2.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador2.exportReport();
                    FileInputStream entrada2 = new FileInputStream(rutafisica2 + "/solicitud.odt");
                    byte[] lectura2 = new byte[entrada2.available()];
                    entrada2.read(lectura2);
                    response.setContentType("application/odt");
                    response.setHeader("Content-Disposition", "attachment; filename=solicitud.odt");
            ...
            Logger.getLogger(SolicitudBlankPrint.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public JasperPrint crearReporte(String telefono, String nombre, String correo, String dependencia, String solicitud, String dni) throws JRException, SQLException, ServletException, ClassNotFoundException, ParserConfigurationException, SAXException, IOException {
        ServletContext srvcon = getServletContext();
        String rutafisica = srvcon.getRealPath("//reportes//");
        ...
        parametro.put(JRParameter.REPORT_LOCALE, locale);
         parametro.put("p_nombre", nombre);
        parametro.put("p_correo", correo);
        parametro.put("p_dependencia", dependencia);
        parametro.put("p_solicitud", solicitud);
        parametro.put("ruta_logo", rutaLogo);
        parametro.put("p_dni", dni);
