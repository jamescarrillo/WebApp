 ...
 * @author alabajos
 */
@WebServlet(name = "ReclamoPrint", urlPatterns = {"/ReclamoPrint"})
public class ReclamoPrint extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            switch (formato) {
                case "pdf":
                    byte[] bs = JasperExportManager.exportReportToPdf(crearReporte(id,anho));
                    ...
                    break;
                case "word":
                    JasperPrint print1 = crearReporte(id,anho);
                    ...
                    exportador1.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica1 + "/reclamo.doc");
                    exportador1.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador1.exportReport();
                    FileInputStream entrada1 = new FileInputStream(rutafisica1 + "/reclamo.doc");
                    byte[] lectura1 = new byte[entrada1.available()];
                    entrada1.read(lectura1);
                    response.setContentType("application/word");
                    response.setHeader("Content-Disposition", "attachment; filename=reclamo.doc");
                ...
                case "excel":
                    JasperPrint print2 = crearReporte(id,anho);
                    ...
                    exportador.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica + "/reclamo.xls");
                    ...
                    FileInputStream entrada = new FileInputStream(rutafisica + "/reclamo.xls");
                    byte[] lectura = new byte[entrada.available()];
                    entrada.read(lectura);
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename = reclamo.xls");
                ...
                case "odt":
                    JasperPrint print3 = crearReporte(id,anho);
                    ...
                    exportador2.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica2 + "/reclamo.odt");
                    exportador2.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador2.exportReport();
                    FileInputStream entrada2 = new FileInputStream(rutafisica2 + "/reclamo.odt");
                    byte[] lectura2 = new byte[entrada2.available()];
                    entrada2.read(lectura2);
                    response.setContentType("application/odt");
                    response.setHeader("Content-Disposition", "attachment; filename=reclamo.odt");
            ...
            Logger.getLogger(ReclamoPrint.class.getName()).log(Level.SEVERE, null, ex);
        }
    }

    public JasperPrint crearReporte(String id, String anho) throws JRException, SQLException, ServletException, ClassNotFoundException, ParserConfigurationException, SAXException, IOException {
        ServletContext srvcon = getServletContext();
        String rutafisica = srvcon.getRealPath("//reportes//");
