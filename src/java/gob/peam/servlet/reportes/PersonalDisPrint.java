 ...
 * @author Ernesto
 */
@WebServlet(name = "PersonalDisPrint", urlPatterns = {"/PersonalDisPrint"})
public class PersonalDisPrint extends HttpServlet {
 public Integer trimestre;
        ...
        try {
            ...
            switch (formato) {
                ...
                case "word":
                    ...
                    exportador1.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica1 + "/discapacidad.doc");
                    exportador1.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador1.exportReport();
                    FileInputStream entrada1 = new FileInputStream(rutafisica1 + "/discapacidad.doc");
                    byte[] lectura1 = new byte[entrada1.available()];
                    entrada1.read(lectura1);
                    response.setContentType("application/word");
                    response.setHeader("Content-Disposition", "attachment; filename=discapacidad.doc");
                ...
                case "excel":
                    ...
                    exportador.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica + "/discapacidad.xls");
                    ...
                    FileInputStream entrada = new FileInputStream(rutafisica + "/discapacidad.xls");
                    byte[] lectura = new byte[entrada.available()];
                    entrada.read(lectura);
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename = discapacidad.xls");
                ...
                case "odt":
                    ...
                    exportador2.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica2 + "/discapacidad.odt");
                    exportador2.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador2.exportReport();
                    FileInputStream entrada2 = new FileInputStream(rutafisica2 + "/discapacidad.odt");
                    byte[] lectura2 = new byte[entrada2.available()];
                    entrada2.read(lectura2);
                    response.setContentType("application/odt");
                    response.setHeader("Content-Disposition", "attachment; filename=discapacidad.odt");
    ...
    public JasperPrint crearReporte() throws JRException, SQLException, ServletException, ClassNotFoundException, ParserConfigurationException, SAXException, IOException {
        ServletContext srvcon = getServletContext();
        String rutafisica = srvcon.getRealPath("//reportes//");
        JasperReport masterReport = (JasperReport) JRLoader.loadObject(rutafisica + "/discapacidad.jasper");
