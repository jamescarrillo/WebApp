 ...
 * @author alabajos
 */
@WebServlet(name = "NotaPrensaPrint", urlPatterns = {"/NotaPrensaPrint"})
public class NotaPrensaPrint extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            switch (formato) {
                case "pdf":
                    byte[] bs = JasperExportManager.exportReportToPdf(crearReporte(idNoticia));
                ...
                case "word":
                    JasperPrint print1 = crearReporte(idNoticia);
                    ...
                    exportador1.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica1 + "/noticia.doc");
                    exportador1.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador1.exportReport();
                    FileInputStream entrada1 = new FileInputStream(rutafisica1 + "/noticia.doc");
                    byte[] lectura1 = new byte[entrada1.available()];
                    entrada1.read(lectura1);
                    response.setContentType("application/word");
                    response.setHeader("Content-Disposition", "attachment; filename=noticia.doc");
                ...
                case "excel":
                    JasperPrint print2 = crearReporte(idNoticia);
                    ...
                    exportador.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica + "/noticia.xls");
                    ...
                    FileInputStream entrada = new FileInputStream(rutafisica + "/noticia.xls");
                    byte[] lectura = new byte[entrada.available()];
                    entrada.read(lectura);
                    response.setContentType("application/vnd.ms-excel");
                    response.setHeader("Content-Disposition", "attachment;filename = noticia.xls");
                ...
                case "odt":
                    JasperPrint print3 = crearReporte(idNoticia);
                    ...
                    exportador2.setParameter(JRExporterParameter.OUTPUT_FILE_NAME, rutafisica2 + "/noticia.odt");
                    exportador2.setParameter(JRExporterParameter.IGNORE_PAGE_MARGINS, true);
                    exportador2.exportReport();
                    FileInputStream entrada2 = new FileInputStream(rutafisica2 + "/noticia.odt");
                    byte[] lectura2 = new byte[entrada2.available()];
                    entrada2.read(lectura2);
                    response.setContentType("application/odt");
                    response.setHeader("Content-Disposition", "attachment; filename=noticia.odt");
            ...
            Logger.getLogger(NotaPrensaPrint.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public JasperPrint crearReporte(Integer id) throws JRException, SQLException, ServletException, ClassNotFoundException, ParserConfigurationException, SAXException, IOException {
        ServletContext srvcon = getServletContext();
        String rutafisica = srvcon.getRealPath("//reportes//");
        JasperReport masterReport = (JasperReport) JRLoader.loadObject(rutafisica + "/noticia.jasper");
