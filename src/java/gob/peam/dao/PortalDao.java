...
public class PortalDao {
    ...
    private static List<ResolucionGerencial> items = new PortalDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new PortalDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        PortalDao.selectedAnho = selectedAnho;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListL", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoL");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, PortalDao.this, "*");
                        }
                    }
                }
            });
