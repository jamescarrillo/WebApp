...
public class DngDao {
    ...
    private static List<ResolucionGerencial> items = new DngDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new DngDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        DngDao.selectedAnho = selectedAnho;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListM", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoM");
        } finally {
            session.close();
        }
    }
    ...
    public void delete(@BindingParam("item") final Integer id) {
        ...
        hm.put("cateId", 2800);
        if (id != null) {
            Messagebox.show("Estas Seguro de Eliminar?", "Dialogo de Confirmación ",
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, DngDao.this, "*");
                        }
                    }
                }
            });
