...
public class IndicadorDao {
    ...
    private static List<ResolucionGerencial> items = new IndicadorDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new IndicadorDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        IndicadorDao.selectedAnho = selectedAnho;
    }
    ...
    public void setSelected(ResolucionGerencial selected) {
        IndicadorDao.selected = selected;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListH", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoH");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, IndicadorDao.this, "*");
                        }
                    }
                }
            });
