...
public class ProyectoDao {
    ...
    private static List<ResolucionGerencial> items = new ProyectoDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new ProyectoDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        ProyectoDao.selectedAnho = selectedAnho;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        hm.put("c", "%" + filter + "%");
        hm.put("anho", "");
        if (!"".equals(anho) && !"".equals(anho)) {
            hm.put("anho", "and trim(substring(docu_fecha_docx from 7)) ='" + anho + "' ");
        }
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListI", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoI");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, ProyectoDao.this, "*");
                        }
                    }
                }
            });
