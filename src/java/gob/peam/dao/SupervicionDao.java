 ...
 * @author alabajos
 */
public class SupervicionDao {

    private SqlSessionFactory sqlSessionFactory;

    public SupervicionDao() {
        sqlSessionFactory = WebAppConnectionFactory.getSqlSessionFactory();
    }
    private static String filter = "";
    private static List<ResolucionGerencial> items = new SupervicionDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new SupervicionDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        SupervicionDao.selectedAnho = selectedAnho;
    }
    ...
    public void setSelected(ResolucionGerencial selected) {
        SupervicionDao.selected = selected;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListB", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoB");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, SupervicionDao.this, "*");
                        }
                    }
                }
            });
