 ...
 * @author alabajos
 */
public class ModificatoriaDao {

    private SqlSessionFactory sqlSessionFactory;

    public ModificatoriaDao() {
        sqlSessionFactory = WebAppConnectionFactory.getSqlSessionFactory();
    }
    private static String filter = "";
    private static List<ResolucionGerencial> items = new ModificatoriaDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new ModificatoriaDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        ModificatoriaDao.selectedAnho = selectedAnho;
    }
    ...
    public void setFilter(String filter) {
        ModificatoriaDao.filter = filter;
    }
    ...
    public void setSelected(ResolucionGerencial selected) {
        ModificatoriaDao.selected = selected;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListJ", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoJ");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, ModificatoriaDao.this, "*");
                        }
                    }
                }
            });
