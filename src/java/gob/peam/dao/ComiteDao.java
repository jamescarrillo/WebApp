...
public class ComiteDao {
    ...
    private static List<ResolucionGerencial> items = new ComiteDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new ComiteDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        ComiteDao.selectedAnho = selectedAnho;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListC", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoC");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, ComiteDao.this, "*");
                        }
                    }
                }
            });
