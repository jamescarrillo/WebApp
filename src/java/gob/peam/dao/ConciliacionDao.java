...
public class ConciliacionDao {
    ...
    private static List<ResolucionGerencial> items = new ConciliacionDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new ConciliacionDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        ConciliacionDao.selectedAnho = selectedAnho;
    }
    ...
    public void setSelected(ResolucionGerencial selected) {
        ConciliacionDao.selected = selected;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListE", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoE");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, ConciliacionDao.this, "*");
                        }
                    }
                }
            });
