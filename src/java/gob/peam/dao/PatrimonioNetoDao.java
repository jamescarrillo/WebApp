 ...
 * @author alabajos
 */
public class PatrimonioNetoDao {

    private SqlSessionFactory sqlSessionFactory;

    public PatrimonioNetoDao() {
        sqlSessionFactory = WebAppConnectionFactory.getSqlSessionFactory();
    }
    private static String filter = "";
    private static List<Finanza> items = new PatrimonioNetoDao().listItems();
    private static String anho = "";
    private static Finanza selected;
    private static List<Anho> anhos = new PatrimonioNetoDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        PatrimonioNetoDao.selectedAnho = selectedAnho;
    }
    ...
    public void setFilter(String filter) {
        PatrimonioNetoDao.filter = filter;
    }
    ...
    public void setSelected(Finanza selected) {
        PatrimonioNetoDao.selected = selected;
    }
    ...
    public Finanza obtenerBean(Integer id) {
        SqlSession session = sqlSessionFactory.openSession();
        HashMap hm = new HashMap();
        hm.put("id", id);
        hm.put("tipo", 6);
        try {
            return session.selectOne("Finanza.get", hm);
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, PatrimonioNetoDao.this, "*");
                        }
                    }
                }
            });
