...
public class EvaluacionDao {
    ...
    private static List<ResolucionGerencial> items = new EvaluacionDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new EvaluacionDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        EvaluacionDao.selectedAnho = selectedAnho;
    }
    ...
    public void setSelected(ResolucionGerencial selected) {
        EvaluacionDao.selected = selected;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListG", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoG");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, EvaluacionDao.this, "*");
                        }
                    }
                }
            });
