...
public class RecomendacionDao {
    ...
    private static List<ResolucionGerencial> items = new RecomendacionDao().listItems();
    private static String anho = "";
    private static ResolucionGerencial selected;
    private static List<Anho> anhos = new RecomendacionDao().getListAnhos();
    ...
    public void setSelectedAnho(Anho selectedAnho) {
        RecomendacionDao.selectedAnho = selectedAnho;
    }
    ...
    public void setFilter(String filter) {
        RecomendacionDao.filter = filter;
    }
    ...
    public void setSelected(ResolucionGerencial selected) {
        RecomendacionDao.selected = selected;
    }
    ...
    public ArrayList<ResolucionGerencial> listItems() {
        ...
        try {
            return (ArrayList) session.selectList("ResolucionObra.getListF", hm);
        } finally {
            session.close();
        }
    }
    ...
    public ArrayList<Anho> getListAnhos() {
        SqlSession session = sqlSessionFactory.openSession();
        try {
            return (ArrayList) session.selectList("ResolucionObra.getAnhoF");
        } finally {
            session.close();
        }
    }
                ...
                public void onEvent(Event evt) {
                    if (evt.getName().equals("onOK")) {
                        ...
                        try {
                            ...
                            sqlSession.close();
                            items = listItems();
                            BindUtils.postNotifyChange(null, null, RecomendacionDao.this, "*");
                        }
                    }
                }
            });
