var mensaje = "Registre en nuestro formulario Las exoneraciones de algunos procesos";
var save = false;
...
function editar() {
    ...
    if ($("#frmRegistro").data("id") != "") {
        ...
        $.ajax({
            dataType: 'json',
            type: 'post',
            url: './Convocatoria?action=GetExoneracion',
            data: {
                "id": $("#frmRegistro").data("id")
            },
            success: function(response) {
                $("#frmRegistro").find("input").eq(0).val(response.nroExonerado);
                $("#frmRegistro").find("input").eq(1).val(response.fecha);
                $("#frmRegistro").find("input").eq(3).val(response.causa);
                $("#frmRegistro").find("textarea").eq(0).val(response.objeto.toString());
                $("#frmRegistro").find("textarea").eq(1).val(response.descripcion.toString());
                $("#frmRegistro").find("input").eq(4).val(response.monto);
                $("#frmRegistro").find("input").eq(5).val(response.contratista);
                $("#frmRegistro").find("input").eq(6).val(response.ruc);
                $("#frmRegistro").find("input").eq(7).val(response.url);
            }
        });
    } else {
...
function actualizar(form, evt) {
    if (!save) {
        ...
        if ($("#frmRegistro").find("textarea").eq(1).val() == "") {
            flag = false;
...
function init() {
    $(function() {
        ...
        $("#frmRegistro").find("input").eq(6).on("keypress", isOnlyNumber);
